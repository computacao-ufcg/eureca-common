...
public class CryptoUtilTest {
    ...
    public void testGetKey() throws IOException {
        ...
        // verify
        Assert.assertEquals(publicKey, actualPubKey);
        Assert.assertEquals(privateKey, actualPrivKey);
    }
    ...
    public void testEncryptDecryptRSA() throws IOException, GeneralSecurityException {
        ...
        // verify
        Assert.assertNotEquals(baseMassage, encryptedMessage);
        Assert.assertEquals(baseMassage, decryptedMessage);
    }
    ...
    public void testSignAndVerify() throws GeneralSecurityException, IOException {
        ...
        // verify
        Assert.assertEquals(expectedignature, actualSignature);
    ...
    public void testGenerateAESKey() {
        ...
        // verify
        Assert.assertEquals(16, randomKey.length());

        for (int i = 0; i < keyChars.length; i++) {
            if (!Character.isLowerCase(keyChars[i])) {
                Assert.fail(String.format(Messages.INVALID_CHAR_C_FOR_RANDOM_KEY_S_AT_INDEX_D, keyChars[i], randomKey, i));
            }
        }
    }
    ...
    public void testConvertKeysToBase64() throws GeneralSecurityException {
        ...
        Assert.assertTrue(privKeyMatcher.matches());
        Assert.assertTrue(pubKeyMatcher.matches());
    }
}
